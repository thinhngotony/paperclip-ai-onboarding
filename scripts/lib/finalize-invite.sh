...
write_start_here_pending() {
	...
	local f="$root_dir/START_HERE.txt"
	{
		...
		echo " ./scripts/bootstrap-ceo-native.sh --force"
		echo ""
...
# If compose_file is empty, runs bootstrap-ceo natively (no Docker)
emit_post_setup_summary() {
	...
	if [[ -z "$compose_file" ]]; then
		...
		local vendor_dir="${root_dir}/vendor/paperclip"
		cd "$vendor_dir"
		pnpm paperclipai auth bootstrap-ceo >"$ilog" 2>&1
	else
		# Docker mode
	...
	if [[ $rc -ne 0 ]]; then
		echo "Could not run bootstrap-ceo (exit $rc). See log below or run ./scripts/bootstrap-ceo-native.sh --force" >&2
		cat "$ilog" >&2
		rm -f "$ilog"
		write_start_here_pending "$root_dir" "(run: ./scripts/bootstrap-ceo-native.sh --force)" "" "$pub"
		return "$rc"
	fi
	...
	if [[ -z "$invite" ]]; then
		echo "Warning: could not parse invite URL from CLI output. Run: ./scripts/bootstrap-ceo-native.sh --force" >&2
		write_start_here_pending "$root_dir" "(run: ./scripts/bootstrap-ceo-native.sh --force)" "" "$pub"
		return 1
	fi
